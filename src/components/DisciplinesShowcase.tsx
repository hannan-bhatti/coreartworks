...
import { Sparkles, Box, Palette, Zap, Layers, ArrowRight } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="space-y-3 max-w-2xl">
            ...
            <p className="text-zinc-400 text-sm sm:text-base leading-relaxed">
              Our multidisciplinary studio bridges imaginative conceptualization, precision 3D production, and real-time engine integration.
            </p>
          </div>
          <div className="text-xs font-mono text-zinc-500 uppercase tracking-wider">
            5 Core Pillars &bull; 21 Sub-Specializations
          </div>
        </div>
              ...
              >
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-white/15 flex items-center justify-center text-white group-hover:border-white group-hover:scale-105 transition-all">
                      <IconComponent className="w-6 h-6" />
                    </div>
                    <span className="text-xs font-mono text-zinc-400 px-2.5 py-1 rounded-full bg-zinc-900/80 border border-white/10">
                      {category.subcategories.reduce((acc, s) => acc + s.itemCount, 0)} Assets
                    </span>
                  </div>
                ...
                <div className="relative z-10 pt-6 mt-6 border-t border-white/10 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {category.subcategories.map((sub) => (
                      <span
                        key={sub.id}
                        className="text-[11px] px-2.5 py-1 rounded-md bg-zinc-900/90 text-zinc-300 border border-white/5 font-medium group-hover:border-white/15 transition-colors"
                      >
                        {sub.name}
                      </span>
                    ))}
                  ...
                  <div className="pt-3 flex items-center justify-between text-xs font-semibold text-white group-hover:text-zinc-300">
                    <span>Explore Discipline Works</span>
